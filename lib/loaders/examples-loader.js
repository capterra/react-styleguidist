...
function examplesLoader(source) {
  ...
  const requiresFromExamples = allCodeExamples.reduce((requires, example) => {
    ...
    const reactRegex = new RegExp(/(?:\/\* react-code-start \*\/)([^]+)(?:\/\* react-code-end \*\/)/, 'i');
    const vanillaRegex = new RegExp(/(?:\/\* vanilla-code-start \*\/)([^]+)(?:\/\* vanilla-code-end \*\/)/, 'i');
    ...
    if (!reactCodeMatch && !vanillaCodeMatch) {
      additionalRequires = requires.concat((0, _getImports.default)(example.content));
    } else if (reactCodeMatch && vanillaCodeMatch) {
      additionalRequires = requires.concat((0, _getImports.default)(reactCodeMatch[1]));
      additionalRequires = additionalRequires.concat((0, _getImports.default)(vanillaCodeMatch[1]));
    } else if (!reactCodeMatch && vanillaCodeMatch) {
      additionalRequires = requires.concat((0, _getImports.default)(vanillaCodeMatch[1]));
    } else if (reactCodeMatch && !vanillaCodeMatch) {
      additionalRequires = requires.concat((0, _getImports.default)(reactCodeMatch[1]));
    }
